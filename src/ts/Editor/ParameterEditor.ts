...
export class ParameterEditor {
  ...
  private static CheckInputValue(value: number): boolean {
    ...
    if (result) {
      debugText('Minimum Value is 0.001.')
    }
  ...
  static InputHeight(ev: Event): void {
    try {
      ...
      let command: Actionable
      if (ParameterEditor.CheckInputValue(+inputElement.value)) {
        command = new ChangeFrameHeight(focusedFrame, 0.001)
      } else {
  ...
  static InputHorAlign(ev: Event): void {
    ...
    if (el.value == 'left' || el.value == 'center' || el.value == 'right') Editor.GetDocumentEditor().projectTree.getSelectedFrame().custom.setHorAlign(el.value)
    else alert('Critical Error: InputHorAlign input type is wrong!')
  }

  static InputVerAlign(ev: Event): void {
    const el = ev.target as HTMLSelectElement
    if (el.value == 'start' || el.value == 'center' || el.value == 'flex-end') Editor.GetDocumentEditor().projectTree.getSelectedFrame().custom.setVerAlign(el.value)
