...
export default abstract class FrameBaseContent implements Saveable {
  ...
  public setHeight(newHeight: number, noChange?: boolean): void {
    ...
    if (!noChange) {
      ...
      const oldHeight = +(this.element.style.height.substring(0, this.element.style.height.length - 2) as string)
      this.element.style.top = `${this.element.offsetTop + (oldHeight - (newHeight * rect.height) / 0.6)}px`
      // this.element.style.top = `${this.element.offsetTop + this.element.style.height - (newHeight * rect.height) / 0.6}px`
